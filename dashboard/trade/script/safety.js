...
window.addEventListener("click", function (event) {
  ...
  document
    .querySelector("#buy_wallet_confirm")
    .addEventListener("click", (e) => {
      ...
      if ((purchaseCost * 1).toFixed(1) * 1 < low_limits * 1) {
        ...
        (purchaseCost * 1).toFixed(2) * 1 >= low_limits * 1 ||
        purchaseCost * 1 <= high_limits * 1
      ) {
        ...
        document
          .querySelector("#buyReceipt")
          .insertAdjacentHTML("afterbegin", buySuccess);
      }
    });
  ...
  const amount_sell = document.querySelector("#sell_wallet #amount"),
    ...
    modal_seller_wallet = document.querySelector(
      "#sell_wallet #seller_wallet p"
    ),
  ...
  document
    ...
    .addEventListener("click", (e) => {
      ...
      e.target.disabled = false;
      if (low_limit.innerHTML * 1 > purchaseCost * 1) {
        e.target.disabled = true;
        ...
        sellFailed.style.display = "block";
        sellFailed.innerHTML =
          "Purchase cost is higher than the high limit Please set a purchase cost that will be lower or equal to the high limit";
        e.target.disabled = true;
        setTimeout(() => {
          sell_cancel.click();
        }, 500);
        ...
        purchaseCost * 1 >= low_limit.innerHTML * 1 ||
        purchaseCost * 1 <= high_limit.innerHTML * 1
      ) {
        let checkFee = removeFee(seller_wallet_buy, purchaseCost);
        console.log(checkFee);
        ...
        document
          .querySelector("#sellReceipt")
          .insertAdjacentHTML("afterbegin", sentSuccess);
      }
    });

  let amount_buy_other = document.querySelector("#buy_other #amount"),
    buy_cancel_other = document.querySelector("#buy_other #buy_other_cancel"),
    ...
    fee_buy_other = document.querySelector("#buy_other #transaction_fee p"),
  ...
  let my_Wallet_buy_other = document.querySelector("#buy_other #my_wallet p"),
    low_limit_buy_other = document.querySelector("#buy_other #low_limit p"),
    high_limit_buy_other = document.querySelector("#buy_other #high_limit p"),
    buyerId_other,
    buyer_wallet,
    order_ref,
    payment_method;
  ...
  setTimeout(() => {
    document.querySelectorAll("#modal-btn_buy_other").forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        wallet_balance_buy_other.innerHTML = `<b>${my_wallet}</b> ${sessionStorage.getItem(
          `${my_wallet}`
        )}`;
        ...
        amount_buy_other.addEventListener("input", (e) => {
          ...
          if (!isNaN(inputValue)) {
            console.log("value is a number");
            //Calculations for the trade and values;
            purchase_cost_buy_other.innerHTML =
              e.target.value * 1 * exchange_rate * 1;
            fee_buy_other.innerHTML = (
              purchase_cost_buy_other.innerHTML *
              1 *
              0.008
            ).toFixed(2);
            receive_amount_buy_other.innerHTML = (
              purchase_cost_buy_other.innerHTML * 1 -
              fee_buy_other.innerHTML * 1
            ).toFixed(3);
        ...
        document.querySelector("#buyMethodDisplay").innerHTML =
          `<b>${payment_method}</b>`.toUpperCase();
  ...
  confirmBtn_buy_other.addEventListener("click", (e) => {
    e.preventDefault();
    console.log(
      high_limit_buy_other.innerHTML.split(" ")[0] * 1,
      purchase_cost_buy_other.textContent * 1
    );
    if (
      amount_buy_other.value * 1 <
      low_limit_buy_other.innerHTML.split(" ")[0] * 1
    ) {
    ...
    sellOrder.append("SellerWallet", seller_wallet);`;
      ...
      formData.append(
        "exchange_rate",
        rate_buy_other.innerHTML.split(" ")[0] * 1
      );
  ...
  const amount_sell_other = document.querySelector(
      "#sell_wallet_other #amount"
    ),
...
`;
  ...
  const success = function (myW, recW, amnt, rcv, ref, tFee) {
    let html = `<div class="name-details">
  <h4>MY WALLET:</h4>
