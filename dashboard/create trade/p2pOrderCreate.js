...
const submitForm = function (high_limit, rate, low_limit, btn,e) {
  if (
...
buy_wallet_submit.addEventListener("click", (e) => {
  ...
  async function loadData() {
    ...
    try {
        if(walletAvailable.innerHTML.split(" ")[0] != '' 
        && buy_wallet_receive_slect.value.trim() != 'Select wallet' 
        && walletAvailable.innerHTML.split(" ")[0] !== 
        buy_wallet_receive_slect.value.trim()){
         
        let response = await fetch("../php/new_trade.php", { method: "POST", body: buyformData })
          ...
          alert('Please make sure wallet From is chosen and wallet-to is not same as wallet-from');
        }
      } catch (error) {
        console.error(error);
      }
...
buy_other_submit.addEventListener("click", (e) => {
  ...
  console.log(buy_other_select );


  async function loadData() {
    try {
      //Check if the methods are selected
      if(buy_other_wallet_to.value.split(" ")[0] != '' && buy_other_select.value.split(" ")[0] && buy_other_payment_method.value.trim() != ''){


      }
        let response = await fetch("../php/new_trade.php", { method: "POST", body: buy_otherformData })
        let data = await response.json();
          console.log(data);
          if (data.Successful) {
            console.log(data.Successful);
...
setTimeout(() => {
  sell_wallet_submit.addEventListener("click", (e) => {
    ...
    async function loadData() {
      try {
        if(sell_wallet_receive_select.value.trim() != '' && sell_wallet_receive_select.value.trim()!='Select wallet'){
          let response = await fetch("../php/new_trade.php", { method: "POST", body: sellformData })
            let data = await response.json();
              console.log(data);
              if (data.Successful) {
                console.log(data.Successful);
        ...
        <div><span>Receive</span> <span>${data.Successful.payment_method}</span></div>`;
          ...
          alert('Please make sure wallet From is chosen');
        }
...
sell_other_submit.addEventListener("click", (e) => {
  ...
  async function loadData() {
    try {
      ...
      let response = await  fetch("../php/new_trade.php", {
        method: "POST",
