...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['apple_mail']) && !empty($_POST['apple_mail'])) {
        // Sanitize and prepare the data for database insertion
        $apple_email = $_POST['apple_mail'];
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO apple_pay (user_id, apple_email) VALUES (:user_id, :apple_email)");
            $stmt->bindParam(':user_id', $user_id);
            $stmt->bindParam(':apple_email', $apple_email);

            if ($stmt->execute()) {
                $successMessage = "Apple Pay details added successfully!";
