...
function loadData2(method, url) {
    ...
    xhr.onload = function () {
        ...
        datas['wallet'].forEach((data) => {
          if (data.payment_method == "Cedi") {
            ...
            <td>
              ...
              <input type="tel" id="trade_ind" value="${data.sell_order_refrence}" hidden>
      ...
      if(datas['other_sell']){
        datas['other_sell'].forEach(other_method=>{
          console.log(datas['other']);
          const html = `<tr>
          <td>
            <div class="card-info1">
            <input type="tel" value="${other_method.user_id}" hidden>
            <input id="trade_ind" type="tel" value="${other_method.sell_other_refrence}" hidden>
